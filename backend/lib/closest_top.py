...
def calculate_indexes_for_xyz(xyz: np.ndarray, chunk_size: int) -> List[np.ndarray]:
    x = xyz[0] - xyz[0] % np.int(chunk_size)
    y = xyz[1] - xyz[1] % np.int(chunk_size)
    z = xyz[2] - xyz[2] % np.int(chunk_size)
...
def calculate_closest_top(xyz: np.ndarray, tops: List[np.ndarray]) -> np.ndarray:
    closest_top = min(tops, key=lambda item: np.linalg.norm(xyz - item))
    return closest_top

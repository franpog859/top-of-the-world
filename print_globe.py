...
def filter_only_tops(xyz_list: list(XYZ)) -> list(XYZ):
    ...
    top_list = []
    for xyz in xyz_list:
        top = get_top_for_direction(CENTER_OF_THE_EARTH, xyz, xyz_list)
...
def get_top_for_direction(center: XYZ, direction: XYZ, xyz_list: list(XYZ)) -> XYZ: 
    xyz_projection_list = []
    for xyz in xyz_list:
        xyz_projection = xyz.project_onto_line(center, direction)
        if are_on_the_same_side_relative_to_center(xyz_projection, direction):
            xyz_projection_list.append(xyz_projection)

    top = max(xyz_projection_list, key=distance)
    return top

...
function Lead({didFetchingFail, isLocationAvailable}:LeadProps) {
  ...
  const textNormal = <>
    <p>Want to go to THE TOP OF THE WORLD but you're too lazy to even think about the Himalayas?</p>
    <p>Well, I've got something for you!</p>
  </>

  const textNoLocation = <>
    <p>Allow the location!</p>
    <p>Click on the <Icon icon={bxsLockAlt} className='Lead-icon' /> near the website address!</p>
  </>

  const textFailedToFetch = <>
    <p>Something went wrong <Icon icon={emojiSad} className='Lead-icon' /></p>
  ...
  return (
    <div className='Lead' data-testid='Lead'>
      <div 
        className='Lead-div Lead-animation'
        key={getRandomKey()}>
          {!isLocationAvailable ? textNoLocation : (!didFetchingFail ? textNormal : textFailedToFetch)}
      </div>
    </div>
  )
}
